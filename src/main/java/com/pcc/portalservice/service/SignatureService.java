...
public class SignatureService {
  ...
  public Object uploadSignature(Long userId, MultipartFile file)
    ...
    User user = userRepository
      .findById(userId)
      .orElseThrow(() ->
        new EntityNotFoundException("ไม่พบผู้ใช้ด้วย ID: " + userId)
      );
    ...
    if (signature == null) {
      signature = Signature.builder().user(user).image(pngImageData).build();
    } else {
  ...
  public byte[] getSignatureImage(Long userId) {
    ...
    if (signature != null) {
      ...
      throw new EntityNotFoundException(
        "ไม่พบลายเซ็นสำหรับผู้ใช้ด้วย ID: " + userId
      );
    }
  }
}
