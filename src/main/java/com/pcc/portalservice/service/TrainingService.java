...
public class TrainingService {
  ...
  public Training setStatusToTraining(
    ...
    if (optionalStatus.isPresent()) {
      ...
      Status statusNull = statusRepository
        .findByTrainingIdAndApproveIdIsNull(trainingId)
        .orElseThrow(() ->
          new RuntimeException("Training not found with ID: " + trainingId)
        );
      if (statusNull.getApproveId() == null) {
        statusNull.setStatus(statusApprove);
        statusNull.setApproveId(approveId);
        statusNull.setActive(1);
        statusRepository.save(statusNull);
      }
    }
  ...
  public List<Map<String, Object>> findTrainingsByApprove1Id(Long approve1Id) {
    String jpql =
      ...
      "FROM status s " +
      "JOIN training t " +
      "ON s.training_id = t.id " +
      "WHERE s.approve_id = :id " +
      "AND s.status != 'ยกเลิก'";

    NativeQuery query = (NativeQuery) entityManager.createNativeQuery(
      jpql,
      Training.class
    );
    ...
    for (Training training : resultList) {
      ...
      for (Status status : training.getStatus()) {
        if (!"ยกเลิก".equals(status.getStatus().toString())) {
          if (status.getStatus() != null) {
            if ("อนุมัติ".equals(status.getStatus().toString())) {
              approvedCount++;
            } else if ("ไม่อนุมัติ".equals(status.getStatus().toString())) {
  ...
  public List<Map<String, Object>> findNextApprove() {
    String jpql =
      "SELECT DISTINCT s.training_id FROM Status s ORDER BY s.training_id";
    ...
    for (Object training : listOfTrainId) {
      String jpqlstatus =
        ...
        "FROM Status s " +
        "LEFT JOIN users u ON s.approve_id = u.id " +
        "WHERE s.training_id = :training AND s.active = 0 " +
        "LIMIT 1;";
      Query statusQuery = entityManager.createNativeQuery(jpqlstatus);
      statusQuery.setParameter("training", training);

      List<String> statusList = statusQuery.getResultList();

      if (statusList == null) {
        statusList.add("รอ personal");
      }
